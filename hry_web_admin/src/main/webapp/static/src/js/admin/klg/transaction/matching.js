	...
	module.exports = {
		...
		listbuy : function() {
			$.ajax({
			 ...
			 //重置按钮
            $("#table_reset_buy").on("click", function () {
                $("#table_query_form_buy")[0].reset();
            });
            //查询按钮
            $("#table_query_buy").on("click", function () {
                var params = $("#table_query_form_buy").serializeJson();
                //查询
                _table.tableQuery($("#table_buy"), params);
            });
            $('#table_buy').on('uncheck.bs.table check.bs.table check-all.bs.table uncheck-all.bs.table',function(e,rows){
                var datas = $.isArray(rows) ? rows : [rows];        // 点击时获取选中的行或取消选中的行
                examine(e.type,datas);                                 // 保存到全局 Set() 里
            });
            var overAllIds = new Array();                // 全局保存选中行的对象
            var smeBuySum = 0;
            var smeBuySumArray = new Array();  //全局数组
            ...
            function examine(type,datas){            
                if(type.indexOf('uncheck')==-1){    
                    $.each(datas,function(i,v){
                       // 添加时，判断一行或多行的 id 是否已经在数组里 不存则添加　
            			...
            			smeBuySum = Number(smeBuySum)+Number(v.smeMoney);
            			});
                }else{
                    $.each(datas,function(i,v){
                ...
                $("#smeBuySum").html("已选买单："+smeBuySum+"SME");
            ...
            $("#pipei").on("click", function () {
			    ...
			    //带参数 刷新
			    $("#table_sell").bootstrapTable('refresh', opt);
			});
            ...
            $("#pipeiConfirm").on("click", function () {
            	$.ajax({
					type : "post",
					url : _ctx + "/klg/transaction/matching/pipei.do?ids="+overAllIds+"&status=1",
					cache : false,
					dataType : "html",
					success : function(data) {
						layer.open({
						   ...
						   success: function(layero, index){
							   /*匹配提交按钮*/
					            $("#peipeiSubmit").on("click", function () {
					            	...
					            	$.ajax({
					    				type : "post",
					    				url : _ctx + "/klg/transaction/matching/peipeiSubmit.do?ids="+overAllIds+"&status=1",
					    				cache : false,
					    				dataType : "json",
					    				success : function(data) {
					    					if (data.success) {
					    						...
					    						layer.closeAll();
								            	loadUrl(_ctx+'/v.do?u=/admin/klg/transaction/matchinglist');
								            	//刷新
								            	/*var opt = {
								    			        url: _ctx + "/klg/transaction/matching/pipeiSelect.do?overAllIds="+overAllIds+"&status=1",
								    			        silent: true,
								            	...
								            	$("#table_sell").bootstrapTable('refresh', opt);
								            	//刷新
								            	$("#table_buy").bootstrapTable('refresh', null);*/
											} else {
            ...
            $("#chidan").on("click", function () {
            	var ids = _table.getIdSelects($("#table_buy"));
            	//alert(ids);
            	if (ids != undefined && ids.length >= 1) {
            		layer.confirm('你确定要吃掉这些订单吗？', {
                        ...
                        $.ajax({
                        	url: _ctx + "/klg/transaction/matching/chidanSubmit.do?ids=" + ids,
                            type: "post",
                            data: {},
                            cache: false,
                            dataType: "json",
		...
		listsell : function() {
            ...
            $("#table_query_sell").on("click", function () {
                ...
                //查询
                _table.tableQuery($("#table_query_sell"), params);
            });
            sellSum = 0;
			var conf = {
				...
				url : _ctx + "/klg/transaction/matching/sellList.do?",
				columns : [ {
					checkbox : true,
					align : 'center',
					valign : 'middle',
