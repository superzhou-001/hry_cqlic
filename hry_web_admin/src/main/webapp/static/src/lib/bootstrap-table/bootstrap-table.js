...
//					that.trigger('search', text_t);
				...
				$search = this.$toolbar.find('.search');
				$search.off('keyup drop').on('keyup drop', 'input',function(event) {
				...
				//下拉框
				var select_search = this.$toolbar.find('.search');
				select_search.on('change','select[lsl]', function(event) {
					debugger
					clearTimeout(timeoutId); // doesn't matter if it's 0
					timeoutId = setTimeout(function() {
