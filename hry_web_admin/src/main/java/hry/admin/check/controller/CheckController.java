...
public class CheckController {
    ...
    public List<Map<String, Object>> culAccountByCustomerErrorAndRightInfosureold(@PathVariable String[] ids) {
        ...
        List<Map<String, Object>> list = remoteAppReportSettlementCheckService.culAccountByCustomersErrorInfosureold(ids, true);
        return list;
    }
    ...
    @ResponseBody
    @CommonLog(name = "余额核算到数据库")
    public JsonResult culAccountByCustomersureold(@PathVariable String[] ids) {
        JsonResult jsonResult = new JsonResult();
        RemoteAppReportSettlementCheckService remoteAppReportSettlementCheckService = SpringUtil.getBean("remoteAppReportSettlementCheckService");
        remoteAppReportSettlementCheckService.culAccountByCustomerssureold(ids);
    ...
    public JsonResult culSureOldAccountAllCustomerErrorInfoToRedis(HttpServletRequest request) {
        ...
        RemoteAppReportSettlementCheckService remoteAppReportSettlementCheckService = SpringUtil.getBean("remoteAppReportSettlementCheckService");
        List<Map<String, Object>> list = remoteAppReportSettlementCheckService.culSureOldAccountAllCustomerErrorInfo(StringUtils.isEmpty(daysstr) ? null : Integer.valueOf(daysstr));
        JsonResult jsonResult = new JsonResult();
        jsonResult.setSuccess(true);
    ...
    public List<Map<String,Object>> culAccountByCustomerErrorInfosureold(@PathVariable String[] ids){
        RemoteAppReportSettlementCheckService remoteAppReportSettlementCheckService = SpringUtil.getBean("remoteAppReportSettlementCheckService");
        List<Map<String,Object>> list=remoteAppReportSettlementCheckService.culAccountByCustomersErrorInfosureold(ids,false);
        return list;
    }
    ...
    public JSONArray culAccountAllCustomerErrorInfo(HttpServletRequest request){
        String str = redisService.get("user_fund_check_all_6.0");
    ...
    public ModelAndView yueinfoview(@PathVariable Long id) {
        ModelAndView mv = new ModelAndView("admin/customer/checkyueinfo");
    ...
    public ModelAndView yueinfoerrorview(@PathVariable Long id) {
        ModelAndView mv = new ModelAndView("admin/customer/checkyueinfoerror");
        mv.addObject("customerId", id);
        return mv;
    }

    @MethodName(name="主动移动机器人的数据")
    @RequestMapping(value="/removeEntrustRobt")
    public void removeEntrustRobt(){
        RemoteAppReportSettlementCheckService remoteAppReportSettlementCheckService = SpringUtil.getBean("remoteAppReportSettlementCheckService");

        remoteAppReportSettlementCheckService.removeEntrustRobt();
    }
}
