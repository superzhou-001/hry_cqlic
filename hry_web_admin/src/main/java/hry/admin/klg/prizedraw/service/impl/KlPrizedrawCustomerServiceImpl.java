...
import hry.admin.klg.prizedraw.dao.KlPrizedrawCustomerDao;
 ...
 * @Date :          2019-06-06 11:31:26  
 */
@Service("klPrizedrawCustomerService")
public class KlPrizedrawCustomerServiceImpl extends BaseServiceImpl<KlPrizedrawCustomer, Long> implements KlPrizedrawCustomerService{
	
	@Resource(name="klPrizedrawCustomerDao")
	@Override
	public void setDao(BaseDao<KlPrizedrawCustomer, Long> dao) {
		super.dao = dao;
	}
	...
	public PageResult findPageBySql(QueryFilter filter) {
		...
		// ----------------------分页查询头部外壳------------------------------
		Page<KlPrizedrawCustomer> page = PageFactory.getPage(filter);
		// ----------------------分页查询头部外壳------------------------------
		...
		((KlPrizedrawCustomerDao) dao).findPageBySql(map);

		return new PageResult(page, filter.getPage(), filter.getPageSize());
	}

	@Override
	public void updateStatusByIssueNumber(Integer issueNumber) {
		// TODO Auto-generated method stub
		Map<String,Object> map = new HashMap<>();
		map.put("issueNumber", issueNumber);
		((KlPrizedrawCustomerDao) dao).updateStatusByIssueNumber(map);
	}
